    ...
    TokenCallbackHandler
{
    ...
    modifier _requireFromEntryPointOrFactory() {
        require(
            msg.sender == address(entryPointCont) ||
    ...
    function _initialize(address[] memory initialOwners) internal {
        require(
            ...
            "[WALLET_INITIALIZE]: atleast one owner required"
        );
    ...
    function executeBatch(
        ...
        require(
            destinations.length == functions.length &&
                values.length == functions.length,
            "[WALLET_EXECUTE_BATCH]: Invalid paramaters"
        );
